...
files.forEach(file => {
    ...
    // This regex grabs everything from `if (supabase) {` up to AND INCLUDING the `}` right before `else if (appwrite) {`
    // Then it grabs `else if (appwrite) {`
    let regex = /(if\s*\(CONFIG\.DB_PROVIDER === 'supabase'\)\s*\{([\s\S]*?)\})(\s*else if\s*\(CONFIG\.DB_PROVIDER === 'appwrite'\)\s*\{)/g;
    
    let matches = 0;
    content = content.replace(regex, (match, supabaseBlock, supabaseInner, appwriteStart) => {
        matches++;
        
        // Find the indent of the appwrite start
        let indentMatch = appwriteStart.match(/\n([ \t]*)/);
        let indent = indentMatch ? indentMatch[1] : '    ';
        
        let cfdbLogic = convertSupabaseToCfDb(supabaseInner, indent);
        
        // Insert CF_DB block BEFORE Appwrite block!
        return supabaseBlock + "\n" + indent + "} else if (CONFIG.DB_PROVIDER === 'cf_db') {\n" + cfdbLogic + "\n" + appwriteStart;
    });
