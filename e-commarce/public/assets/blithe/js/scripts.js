...
$(function () {
  ...
  var toggleLayout = function () {
    ...
    if (w.outerWidth() <= 1024) {
      ...
      // MOBILE SIDEBAR — FORCE NEW SYSTEM
      $("body").removeClass("layout-2 layout-3 sidebar-mini sidebar-show sidebar-gone");
      
      // Prevent framework from breaking our new button — ONE-TIME handler
      if (!window.__mobileMenuBtnBound) {
        window.__mobileMenuBtnBound = true;
        $(document).on('click', '#mobile-menu-btn', function(e) {
            ...
            if (typeof window.toggleMobileSidebar === 'function') {
              ...
              document.body.style.overflow = document.body.classList.contains('mobile-sidebar-active') ? 'hidden' : '';
            }
            return false;
        });
      }
...
function setTheme(themeName) {
  localStorage.setItem("blithe_theme", themeName);
...
function toggleTheme() {
  var currentTheme = localStorage.getItem("blithe_theme");
...
(function () {
  if (localStorage.getItem("blithe_theme") === "dark") {
    setTheme("dark");
  } else {
    setTheme("light");
  }
